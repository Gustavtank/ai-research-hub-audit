...
RESEARCH/RUNS contains completed or audited research and may be reused
as stronger research evidence, while still remaining distinct from
approved canonical KNOWLEDGE.

RESEARCH/WORK may also be consulted when relevant, including completed
work that has not yet been promoted to RESEARCH/RUNS. Material from
RESEARCH/WORK is non-canonical evidence: it may provide context,
sources, leads, prior findings, or contradictions, but must never be
treated as approved truth merely because it exists locally.

When RESEARCH/WORK and RESEARCH/RUNS disagree, the disagreement must be
identified and evaluated against current external evidence.
...
INDEX
  ↓
GENERATED discovery indexes when INDEX is empty, incomplete, or insufficient
  ↓
relevant KNOWLEDGE
  ↓
relevant SOURCES
  ↓
relevant RESEARCH/RUNS
  ↓
relevant RESEARCH/WORK when useful
  ↓
external research

The canonical INDEX remains the first navigation layer.

The generated discovery files:

- /hub/GENERATED/indexes/hub-index.json
- /hub/GENERATED/indexes/hub-tree.md

may be used to discover existing files, runs, topics, and locations when
the canonical INDEX does not provide enough navigation information.

GENERATED discovery indexes are navigation aids only. They are not
canonical knowledge and must not be treated as factual evidence merely
because information appears in them.

When RESEARCH/WORK contains potentially relevant previous research,
listing filenames or run directories does not count as consulting that
research.

To treat a previous RESEARCH/WORK run as consulted, the agent must read
enough of its actual content to determine relevance. Normally this means:

1. read question.md when available;
2. read report.md when relevant;
3. read sources.md when provenance, comparison, or verification would
   materially improve the current research.

The agent must not conclude that no relevant local research exists merely
because INDEX, KNOWLEDGE, SOURCES, or RESEARCH/RUNS are empty before
checking relevant RESEARCH/WORK when the discovery indexes or filesystem
show candidate runs there.

When INDEX or GENERATED discovery indexes do not identify relevant prior
research with sufficient confidence, the agent must perform a targeted
recursive content search in RESEARCH/RUNS and RESEARCH/WORK before
concluding that no relevant local research exists.

This recursive search must not assume a fixed directory depth.
RESEARCH/WORK may contain an agent directory followed by one or more run
directories before the research files themselves.

For discovery, prioritize actual research content in this order when
available:

1. question.md;
2. report.md for candidate runs;
3. sources.md when provenance or comparison is useful.

Listing filenames or directories alone does not satisfy this fallback
search. The agent must inspect the content of relevant candidate files.
